...
export interface StoreEntry {
	id: string;    // 8 hex character id
	hash: number;  // CRC16 from the server's "hash" property
	data: Entity;
}
...
export function nextTableSize(entryCount: number, loadFactor = DEFAULT_LOAD_FACTOR): number {
...
let indexView: DataView | null = null;
let tableSize = 0;
let entryCount = 0;
...
function requireIndex(): DataView {
	ensureIndex();
	return indexView as DataView;
}
...
export function ensureIndex(): void {
	...
	const header = file.read(ArrayBuffer, HEADER_SIZE) as ArrayBuffer;
	const headerView = new DataView(header);
	entryCount = headerView.getUint32(0, false);
	tableSize = headerView.getUint32(4, false);

	indexBuffer = file.read(ArrayBuffer, tableSize * SLOT_SIZE) as ArrayBuffer;
	indexView = new DataView(indexBuffer);
	file.close();
}

export function getStats(): { entryCount: number; tableSize: number; loadFactor: number } {
	ensureIndex();
...
export function lookupEntity(id: number): LookupResult | null {
	...
	for (let probes = 0; probes < tableSize; probes++) {
		...
		if (slotId === EMPTY_ID) return null;
...
export function getEntity(id: string): Entity | null {
	...
	const text = file.read(String, result.length) as string;
	file.close();
...
export function verifyEntity(id: string): boolean {
	const result = lookupEntity(idToNumber(id));
...
export function putEntity(id: string, data: Entity, hash?: number): void {
	const view = requireIndex();
	...
	const resolvedHash = hash ?? new CRC16(0x1021).checksum(bytes);
...
interface RawRecord { id: number; hash: number; text: string }

function writeStore(records: RawRecord[], expectedCount = records.length, loadFactor = DEFAULT_LOAD_FACTOR): void {
	...
	for (const r of records) totalSize += r.text.length;
	...
	for (const r of records) {
		...
		for (let i = 0; i < r.text.length; i++) bytes[dataPos + i] = r.text.charCodeAt(i);
		dataPos += r.text.length;
	}

	Directory.create(STORE_DIR);
	if (File.exists(STORE_PATH)) File.delete(STORE_PATH);
	const file = new File(STORE_PATH, true);
	file.write(buf);
	file.close();
...
export function initStore(expectedEntityCount: number, loadFactor = DEFAULT_LOAD_FACTOR): void {
...
export function buildStore(entries: StoreEntry[], loadFactor = DEFAULT_LOAD_FACTOR): void {
...
export function compact(expectedEntityCount?: number, loadFactor = DEFAULT_LOAD_FACTOR): void {
	const view = requireIndex();
	const file = new File(STORE_PATH);
	...
	for (let slot = 0; slot < tableSize; slot++) {
		...
		file.position = offset;
		const text = file.read(String, length) as string;
		records.push({ id, hash, text });
	}
	file.close();

	writeStore(records, expectedEntityCount ?? records.length, loadFactor);
}
...
function encodeStateRecord(name: string, id: string): string {
	if (name.length > 255) throw new Error("state name must be 255 characters or fewer");
	return String.fromCharCode(name.length) + name + id;
}

function decodeStateRecordName(record: string): string {
	return record.slice(1, 1 + record.charCodeAt(0));
}

function decodeStateRecordId(record: string): string {
	const len = record.charCodeAt(0);
	return record.slice(1 + len, 1 + len + 8);
}

let stateIndexBuffer: ArrayBuffer | null = null;
let stateIndexView: DataView | null = null;
let stateTableSize = 0;
let stateEntryCount = 0;
const stateCache = new Map<string, string>();

export function stateStoreExists(): boolean {
	return File.exists(STATE_PATH);
}

// Loads the header + index region into PSRAM. Cheap to call repeatedly —
// only reads from flash the first time.
export function ensureStateIndex(): void {
	if (stateIndexView) return;
	...
	const header = file.read(ArrayBuffer, STATE_HEADER_SIZE) as ArrayBuffer;
	const headerView = new DataView(header);
	stateEntryCount = headerView.getUint32(0, false);
	stateTableSize = headerView.getUint32(4, false);

	stateIndexBuffer = file.read(ArrayBuffer, stateTableSize * STATE_SLOT_SIZE) as ArrayBuffer;
	stateIndexView = new DataView(stateIndexBuffer);
	file.close();
}

export function getStateStats(): { entryCount: number; tableSize: number; loadFactor: number } {
	ensureStateIndex();
	return { entryCount: stateEntryCount, tableSize: stateTableSize, loadFactor: stateEntryCount / stateTableSize };
}

export function getStateId(name: string): string | null {
	const cached = stateCache.get(name);
	if (cached !== undefined) return cached;

	ensureStateIndex();
	const view = stateIndexView as DataView;
	...
	for (let probes = 0; probes < stateTableSize; probes++) {
		...
		if (slotHash === hash) {
			...
			const length = view.getUint16(pos + 8, false);
			file.position = offset;
			const record = file.read(String, length) as string;
...
function findStateSlot(file: File, view: DataView, size: number, hash: number, name: string): { slot: number; isNew: boolean } {
	let slot = hash % size;
	for (let probes = 0; probes < size; probes++) {
		const pos = slot * STATE_SLOT_SIZE;
		const slotHash = view.getUint32(pos, false);

		if (slotHash === EMPTY_HASH) return { slot, isNew: true };
		...
		if (slotHash === hash) {
			...
			file.position = offset;
			const record = file.read(String, length) as string;
			if (decodeStateRecordName(record) === name) return { slot, isNew: false };
		}
...
export function setStateId(name: string, id: string): void {
	ensureStateIndex();
	const view = stateIndexView as DataView;
	const hash = fnv1a(name);
...
export function getState(name: string): Entity | null {
	const id = getStateId(name);
	return id ? getEntity(id) : null;
}

interface RawStateRecord { name: string; id: string }

function writeStateStore(entries: RawStateRecord[], expectedCount = entries.length, loadFactor = DEFAULT_LOAD_FACTOR): void {
	...
	Directory.create(STORE_DIR);
	if (File.exists(STATE_PATH)) File.delete(STATE_PATH);
	const file = new File(STATE_PATH, true);
	file.write(buf);
	file.close();
...
export function initStateStore(expectedNameCount: number, loadFactor = DEFAULT_LOAD_FACTOR): void {
...
export function compactStateStore(expectedNameCount?: number, loadFactor = DEFAULT_LOAD_FACTOR): void {
	const view = requireStateIndex();
	const file = new File(STATE_PATH);

	const records: RawStateRecord[] = [];
	...
	writeStateStore(records, expectedNameCount ?? records.length, loadFactor);
}

function requireStateIndex(): DataView {
	ensureStateIndex();
	return stateIndexView as DataView;
}
